...
void move_to_MRU_hot(struct List * source,struct Node * node);
...
struct Page * buffer_request_page(int file_id, long block_id, char operation){
	...
	if(page != NULL){ //HIT - Update MRU
		...
		if(node->list == cold){
            move_to_MRU_hot(cold, node); // moves the page from COLD to HOT MRU
        }  else if (node->list == hot){
            move_to_MRU_hot(hot, node); // moves the page from HOT to HOT MRU
        }
		...
		if (buffer_is_full() == FALSE) {
			...
			insert_MRU(cold, new_node); //Insert in Cold MRU
			...
			if (cold->size > MIN_LC) { //Check cold list's size
